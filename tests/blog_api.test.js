...
describe('when there is initially some blogs saved', () => {
  ...
  describe('addition of a new blog by existing logged-in user', () => {
    ...
    test('if added blog is missing likes then zero likes are given', async () => {
      ...
      const blogsAtEnd = await helper.blogsInDb()
      const newBlogAtEnd = blogsAtEnd.find(blog => blog.title === 'The 2030 Self-Driving Car Bet')
...
describe('when there is initially one user at db', () => {
  ...
  test('creation fails with proper statuscode and message if username or password is too short or missing', async () => {
    ...
    expect(result.body.error).toContain('User validation failed: username: Path `username` (`gg`) is shorter than the minimum allowed length (3).')
    expect(result2.body.error).toContain('User validation failed: username: Path `username` is required.')
    expect(result3.body.error).toContain('password missing or too short, must be at least 3 characters')
    expect(result4.body.error).toContain('password missing or too short, must be at least 3 characters')
